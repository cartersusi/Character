...
int main() {
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        for (int i = 0; i <= Screen::w / ground_floor_size; i++) {
            ...
            model = glm::translate(model, glm::vec3(ground_floor_size * i, _Character::MIN_GROUND_Y - (ground_floor_size/4), 0.0f));
            model = glm::scale(model, glm::vec3(ground_floor_size, ground_floor_size, 1.0f));
            glUniformMatrix4fv(glGetUniformLocation(shader_program, "model"), 1, GL_FALSE, glm::value_ptr(model));
            ...
            model = glm::mat4(1.0f);
            model = glm::translate(model, glm::vec3(ground_floor_size * i, _Character::MIN_GROUND_Y+(ground_floor_size/4), 0.01f));
